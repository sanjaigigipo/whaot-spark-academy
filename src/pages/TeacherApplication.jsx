...
const TeacherApplicationForm = () => {
  ...
  const updateFormData = (questionId, value) => {
  ...
  const renderInput = (id, label, type='text', required=false, placeholder='', options=[]) => {
    ...
    return (<div className="space-y-2" key={id}>
      ...
      {['text','email','number'].includes(type)&&<Input id={id} type={type} value={value}
        onChange={(e)=>updateFormData(id,e.target.value)}
        placeholder={placeholder} className={error?'border-red-500':''}/>}
      {type==='textarea'&&<Textarea id={id} value={value}
        onChange={(e)=>updateFormData(id,e.target.value)}
        placeholder={placeholder} className={`min-h-[100px] ${error?'border-red-500':''}`} rows={3}/>}
      {type==='select'&&<Select value={value} onValueChange={val=>updateFormData(id,val)}>
  ...
  const renderRadioGroup=(id, label, options, required=false) => {
    ...
    return (<div className="space-y-3" key={id}>
      ...
      <RadioGroup value={value} onValueChange={val=>updateFormData(id,val)} className="grid grid-cols-1 gap-2">
